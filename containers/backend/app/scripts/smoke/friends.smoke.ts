...
const FRIENDSHIP_TEST_EMAIL =
  process.env.FRIENDSHIP_TEST_EMAIL ?? "pikachu@seed.local";
...
async function testListPendingSent(): Promise<void> {
  ...
  assert(
    ...
    "Sent pending should include charmander",
  );
}
...
async function testDuplicateRequestRejected(): Promise<void> {
  ...
  assert(
    send.res.status === 409,
    "Duplicate pending request should return 409",
  );
...
async function testReverseRequestAutoAccept(): Promise<void> {
  ...
  assert(
    send.res.status === 200,
    "Reverse request auto-accept should return 200",
  );
...
async function main(): Promise<void> {
  ...
  await runTest(
    results,
    "send request to isolated user",
    testSendRequestToIsolatedUser,
  );
  await runTest(
    results,
    "duplicate request rejected",
    testDuplicateRequestRejected,
  );
  await runTest(
    results,
    "reverse request auto-accept",
    testReverseRequestAutoAccept,
  );
