...
function toValidationDetails(error: z.ZodError): ValidationErrorDetails {
  ...
  for (const issue of error.issues) {
    ...
    const code: ValidationCode = (Object.values(VALIDATION) as string[]).includes(msg)
...
function zodErrRes<TSchema>(res: Response, parsed: z.ZodSafeParseError<ZodInfer<TSchema>>): void {
...
export function validateBody<TSchema extends ZodType<unknown>>(schema: TSchema) {
