...
export function splitScreenGridStyles({
  ...
  const mobileFullHeight = mobileFullClassName ?? (isMobileSplit ? 'min-h-[60dvh]' : 'h-dvh');
  const mobileSideHeight = mobileSideClassName ?? (isMobileSplit ? 'min-h-[40dvh]' : 'h-dvh');
  const mobileSidePosition =
    mobileSideLayout === 'overlay' ? 'absolute inset-0 h-dvh' : mobileSideHeight;
  const mobileSidePointerEvents = mobileSideInteractive
