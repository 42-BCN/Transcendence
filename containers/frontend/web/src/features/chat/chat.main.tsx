...
function getInvitationCardState(args: {
	...
	const isAccepted =
		status === "accepted" || typeof message.content.acceptedAt === "string";
	const isCancelled =
		status === "cancelled" || typeof message.content.cancelledAt === "string";
...
function renderInvitationCard(args: {
	...
	const action = shouldShowAcceptAction({
		state,
		hasActiveGameRoom,
		onAcceptInvitation,
	})
		? renderInvitationActions({
				...
				joiningError:
					joiningError && joiningErrorInvitationId === message.id
						? joiningError
						: null,
				decliningError:
					decliningError && decliningErrorInvitationId === message.id
						? decliningError
						: null,
				onAcceptInvitation,
				onDeclineInvitation,
	...
	useEffect(() => {
		...
		if (initialUnreadMessageId && !didScrollToUnreadRef.current) {
			const target = document.getElementById(
				`message-${initialUnreadMessageId}`,
			);
				...
				{messages.map((message) => {
					...
					const messageClassName =
						type === "system" || type === "error"
							? chatStyles.main.metaRow
							: undefined;
					const textVariant =
						type === "system" || type === "error" ? "caption" : "body-xs";
					const textClassName =
						type === "system" || type === "error"
