...
function AbButton(name: string) {
  const selectAbility = useGame((state) => state.selectAbility);
  const unselectAbility = useGame((state) => state.unselectAbility);
...
function Scene() {
  ...
  const { tiles, entities, width, height, depth } = info;
  ...
  useEffect(() => {
    init(entities, tile, mapInfo);
  }, [entities, mapInfo, tiles]);
  return (
    <>
