...
- [Project Board View](https://github.com/orgs/42-BCN/projects/1/views/5) — GitHub Projects view used to organize and monitor feature work.
...
2. **Launch the application**:
   _Note: You **do not** need to run `make setup` manually. The Makefile will automatically run the environment configuration and generate the required HTTPS certificates on-the-fly when starting any environment._
   ...
   The compose stack uses service-specific env files generated by `scripts/env/setup-env.sh`. The generated files live in:

   - `containers/nginx/.env.development` and `containers/nginx/.env.production`
   - `containers/backend/docker/.env.development` and `containers/backend/docker/.env.production`
   - `containers/socket/docker/.env.development` and `containers/socket/docker/.env.production`
   - `containers/frontend/docker/.env.development` and `containers/frontend/docker/.env.production`
   - `containers/database/docker/.env.development` and `containers/database/docker/.env.production`
   - `containers/cloudflared/.env.development` and `containers/cloudflared/.env.production`

   Tracked templates live alongside them as `*.example` files.
...
_Access the Storybook dev portal at: `http://localhost:6006`_
...
- [SwaggerHub API Docs](https://app.swaggerhub.com/apis-docs/carolinapapes/backend-api/1.0.0?view=uiDocs#/Internal/postInternalGameInvitationsMarkJoined) — Interactive Swagger UI for browsing and testing the published API documentation.
