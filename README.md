...
2. **Launch the application**:
   *Note: You **do not** need to run `make setup` manually. The Makefile will automatically run the environment configuration and generate the required HTTPS certificates on-the-fly when starting any environment.*
   ...
   The project currently supports two main runtime flows:

   #### A. Evaluation Flow
   This is the default `make` target. It prepares both development and production env files, starts the development stack to populate the shared database, switches to the production stack using that same data, and then launches a production Cloudflare Quick Tunnel.
   ```bash
   make
   ```
   This expands to:
   ...
   The result is a production build running against the database that was created and seeded from the development flow. The tunnel URL is printed in the terminal when Cloudflare is ready.

   #### B. Production Mode
   Runs the production stack only.
   ```bash
   make prod
   ```
   *Access the app at: `https://localhost:8443`*
   ...
   #### C. Development Mode
   Runs all services with hot-reloading, generates the development env files automatically, pushes the Prisma schema, and seeds the database.
   ```bash
   make dev
   ```
   *Access the app at: `https://localhost:8443`*
...
The development and production compose stacks point at the same Docker database volume. That means you can seed/populate data in development and then reuse it when switching to production.

- `make switch-prod` — stops the development stack and starts the production stack
- `make switch-dev` — stops the production stack and starts the development stack

This is the same mechanism used by the default `make` target.
...
2. **Find the public URL**:
   ...
   ```bash
   make prod-tunnel-quick-logs
   ```
   Look for the printed line containing `https://[your-random-subdomain].trycloudflare.com`.
3. **Google OAuth Callback Adjustment**:
   Because the tunnel URL changes every time you launch it, if you plan to test Google OAuth login, update the authorized redirect URI in your Google Developer Console to:
   `https://[your-random-subdomain].trycloudflare.com/api/v1/auth/google/callback`
4. **Shutdown the tunnel**:
   ```bash
   make prod-tunnel-quick-down
   ```
...
**Main flows**
- `make`: Full evaluation flow: setup development env, setup production env, run `dev`, switch to production, then start a production quick tunnel
- `make dev`: Development stack with schema push and seed
- `make prod`: Production stack only
